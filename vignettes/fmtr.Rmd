...
#### The `fapply()` Function
The simplest introduction to the **fmtr** package is to examine the use of
the `fapply()` function.  
...
takes two parameters: a vector and a format.  In this way, the function acts
...
For a brief introduction to formatting string codes, see `FormattingStrings`.

And here is an example showing the use of a lookup list:
```
library(fmtr)
...
```
Notice in the above example that the user-defined format allows you 
...
with data frames and tibbles instead of vectors.  
...
This method of formatting provides much greater control over data formatting
...
of a lookup style format for the state names, and a vectorized format function
for the state area.
...
The format catalog can then be read into any number of programs using 
...
* [Format Data Function](https://fmtr.r-sassy.org/articles/fmtr-fdata.html)
* [Format Apply Function](https://fmtr.r-sassy.org/articles/fmtr-fapply.html)
* [Format Catalogs](https://fmtr.r-sassy.org/articles/fmtr-fcat.html)
* [Convenience Functions](https://fmtr.r-sassy.org/articles/fmtr-convenience.html)
* [Helper Functions](https://fmtr.r-sassy.org/articles/fmtr-helpers.html)
